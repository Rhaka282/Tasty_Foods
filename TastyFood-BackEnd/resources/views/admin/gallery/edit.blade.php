            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                <div class="p-8 text-gray-900">
                    <form action="{{ route('admin.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <!-- Title -->
                        <div>
                            <label for="title" class="block text-sm font-bold text-gray-700 uppercase tracking-wider mb-2">Nama / Judul Hidangan</label>
                            <input type="text" name="title" id="title" class="w-full rounded-xl border-gray-200 focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50 transition-shadow py-3 px-4 text-sm font-semibold" placeholder="Contoh: Salad Sayur Madu" value="{{ old('title', $gallery->title) }}">
                        </div>

                        <!-- Image -->
                        <div>
                            <label for="image" class="block text-sm font-bold text-gray-700 uppercase tracking-wider mb-2">Ubah Foto Makanan (Opsional)</label>
                                ...
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-semibold text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                            <span>Pilih foto makanan baru</span>
                                            <input id="file-upload" name="image" type="file" class="sr-only" accept="image/*" onchange="previewImage(event)">
                                        </label>
                                        <p class="pl-1">atau seret dan lepas</p>
                                    </div>
                                    <p class="text-xs text-gray-400">PNG, JPG, JPEG hingga 2MB (Biarkan kosong jika tidak ingin mengubah foto)</p>
                                </div>
                            </div>
                            <!-- Image Preview -->
                            <div id="preview-container" class="mt-4 text-center">
                                <p class="text-xs text-gray-400 mb-2">Foto Saat Ini / Pratinjau:</p>
                                <img id="image-preview" src="{{ str_starts_with($gallery->image, 'ASET/') ? 'http://127.0.0.1:8000/'.$gallery->image : asset('storage/'.$gallery->image) }}" alt="Pratinjau" class="mx-auto max-h-48 rounded-lg shadow-sm border border-gray-100 object-cover">
                            </div>
                        </div>
